...
namespace GestorDeCitas
{
    ...
    {
        private string connectionString,fecha,total;
        private DataTable dt = new DataTable("Corte");
        private int sum = 0;
        ...
        private void metroSetButton1_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = "DELETE FROM Corte WHERE Fecha LIKE '%' + @fecha + '%' ";

                using (SQLiteConnection con = new SQLiteConnection(connectionString))
                using (SQLiteCommand deleteRecord = new SQLiteCommand(sql, con))
                {
                    con.Open();
                    deleteRecord.Parameters.Add(new SQLiteParameter("@fecha", this.metroSetComboBox1.GetItemText(this.metroSetComboBox1.SelectedItem)));
